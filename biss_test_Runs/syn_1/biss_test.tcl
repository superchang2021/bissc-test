...
set rc [catch {
  ...
  elaborate -top {biss_test}
  export_db {biss_test_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../01_src/04_pin/biss_test.adc
  optimize_rtl
  report_area -file biss_test_rtl.area
  export_db {biss_test_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  ...
  optimize_gate -maparea biss_test_gate.area
  legalize_phy_inst
  export_db {biss_test_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
